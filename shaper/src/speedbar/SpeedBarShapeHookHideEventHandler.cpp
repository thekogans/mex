...
namespace thekogans {
    namespace mex {
        namespace shaper {
            ...
            struct SpeedBarShapeHookHideEventHandler : public core::SpeedBar::Item::ButtonEventHandler {
                THEKOGANS_MEX_CORE_DECLARE_SPEEDBAR_ITEM_EVENT_HANDLER (SpeedBarShapeHookHideEventHandler)

                core::Module &module;

                explicit SpeedBarShapeHookHideEventHandler (core::Module &module_) :
                    module (module_) {}

                virtual void OnSetFocus () {
                    if (core::GetIOProject ().shaper.hook.visible) {
                        core::GetIOProject ().shaper.hook.visible = false;
                        for (util::ui32 i = 0, count = core::GetIOProject ().shaper.viewLayout.GetViewCount (); i < count; ++i) {
                            ...
                            if (openglView.get () != 0) {
                                ...
                                _3ds::opengl::Hook (core::GetIOProject ().shaper.hook).DrawWithColor (*openglView);
                            }
                        }
            ...
            THEKOGANS_MEX_CORE_IMPLEMENT_SPEEDBAR_ITEM_EVENT_HANDLER (SpeedBarShapeHookHideEventHandler, Shaper)
