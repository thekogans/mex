...
namespace thekogans {
    namespace mex {
        namespace shaper {
            ...
            THEKOGANS_MEX_COMMAND_IMPLEMENT_COMMAND (DrawPolygonsCommand)

            bool DrawPolygonsCommand::Undo () {
                if (view.IsShape () && view.IsEnabled ()) {
                    ...
                    if (openglView.get () != 0) {
                        ...
                        for (std::size_t i = 0, count = bezierPolygons.size (); i < count; ++i) {
                            _3ds::opengl::BezierPolygon2 (
                                *bezierPolygons[i],
                                _3ds::opengl::BezierPolygon2::ShowVertices,
                                core::GetIOProject ().shaper.steps,
                                _3ds::io::Lofter::ModelParams::High,
                                _3ds::io::BezierPolygon2::Vertex::GetSelectionSet (
                                    core::GetIOProject ().shaper.selectMask)).DrawWithColor (
                                        *openglView,
                                        _3ds::opengl::DefaultPalette[_3ds::opengl::MEDIUM_GRAY]);
                        }
                    }
                }
                return true;
            ...
            bool DrawPolygonsCommand::Redo () {
                if (view.IsShape () && view.IsEnabled ()) {
                    ...
                    if (openglView.get () != 0) {
                        ...
                        for (std::size_t i = 0, count = bezierPolygons.size (); i < count; ++i) {
                            _3ds::opengl::BezierPolygon2 (*bezierPolygons[i], flags, core::GetIOProject ().shaper.steps,
