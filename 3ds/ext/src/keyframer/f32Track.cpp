    ...
    namespace mex {
        namespace _3ds {
            namespace ext {
                ...
                f32Track::f32Track (const io::f32Track &track_, const io::Keyframer::Segment &segment) : track (track_) {
                    {
                        // vectorize
                        for (std::size_t i = 0, count = track.keys.size (); i < count; ++i) {
                            keys.push_back (f32Key (track.keys[i]));
                        }
                    }
                    if (keys.size () > 2) {
                        std::size_t i, count;
                        ...
                        if (track.IsLoops ()) {
                            keys[0].CompDerivFirstLoop (keys[i - 1], keys[1], static_cast<util::f32> (segment.length));
                            keys[i].CompDerivLastLoop (keys[i - 1], keys[1], static_cast<util::f32> (segment.length));
                        }
                        else {
                ...
                util::f32 f32Track::GetTweenHelper (util::ui32 frame) const {
                    if (keys.empty ()) {
                        return 0.0f;
                    }
                    if (track.IsRepeats ()) {
