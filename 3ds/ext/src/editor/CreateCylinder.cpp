    ...
    namespace mex {
        namespace _3ds {
            namespace ext {
                ...
                io::Mesh::UniquePtr Mesh::CreateCylinder (
                    ...
                    if (mesh.get () != 0) {
                        ...
                        util::ui32 vertexIndex = 0;
                        {
                            ...
                            for (util::ui32 i = 0; i <= segs; ++i, height -= dheight) {
                                util::f32 theta = 0.0f;
                                for (util::ui32 j = 0; j < bands; ++j, theta += dtheta) {
                                    vertices[vertexIndex++] = blas::Point3 (radius * cosf (theta), radius * sinf (theta), height);
                                }
                            }
                            height += dheight;
                            vertices[vertexIndex++] = blas::Point3 (0.0f, 0.0f, height);
                            assert (vertexIndex == (segs + 1) * bands + 2);
                        }
                        ...
                        {
                            util::ui32 lastBandStartIndex = bands * segs + 1;
                            for (util::ui32 i = 0; i < bands; ++i) {
                                faces[faceIndex++] = io::Mesh::Face (
                            ...
                            assert (faceIndex == (segs + 1) * bands * 2);
                        }
                        if (smooth) {
