    ...
    namespace mex {
        namespace editor {
            ...
            namespace {
                ...
                private:
                    const std::vector<_3ds::io::Camera *> &cameras;
                ...
                protected:
                    virtual BOOL OnInitDialog () {
                        ...
                        for (std::size_t i = 0, count = cameras.size (); i < count; ++i) {
                            ((CListBox *)wnd)->AddString (cameras[i]->name.c_str ());
                        }
                ...
                private:
                    const std::vector<_3ds::io::Light *> &lights;
                ...
                protected:
                    virtual BOOL OnInitDialog () {
                        ...
                        for (std::size_t i = 0, count = lights.size (); i < count; ++i) {
                            if (lights[i]->spot.get () != 0) {
                                ((CListBox *)wnd)->AddString (lights[i]->name.c_str ());
                            }
                        }
                        ((CListBox *)wnd)->SetCurSel (0);
                        return TRUE;
                ...
                private:
                    core::Module &module;
                    _3ds::io::Editor &editor;
                    util::ui16 currStyle;
                    char currView;
                    const _3ds::io::Light *spotLight;
                    const _3ds::io::Camera *camera;
                    char oldViews[4];
                    char newViews[4];
                    const _3ds::io::Light *spotLights[4];
                    const _3ds::io::Camera *cameras[4];
...
#define TTY (T + 2 * H / 3)
                        ...
                        for (util::ui32 i = 0; i < 4; ++i) {
                            ...
                            switch (viewLayout[i].type) {
                                ...
                                case _3ds::io::View::SpotLight:
                                    oldViews[i] = '$';
                                    newViews[i] = '$';
                                    spotLights[i] = editor.GetLight (viewLayout[i].name);
                                    assert (spotLights[i] != 0);
                                    break;
                                case _3ds::io::View::Camera:
                                    oldViews[i] = 'C';
                                    newViews[i] = 'C';
                                    cameras[i] = editor.GetCamera (viewLayout[i].name);
                                    assert (cameras[i] != 0);
                                    break;
                            }
                        }
                ...
                protected:
                    inline util::ui32 GetSpotLightCount () const {
                        util::ui32 spotLightCount = 0;
                        const std::vector<_3ds::io::Light *> &lights = editor.lights;
                        for (std::size_t i = 0, count = lights.size (); i < count; ++i) {
                            if (lights[i]->spot.get () != 0) {
                                ++spotLightCount;
                            }
                        ...
                        module.commandDispatcher.Dispatch (
                            ViewLayoutSetStyleCommand::UniquePtr (
                                new ViewLayoutSetStyleCommand (
                                    editor, currStyle, oldViews,
                                    newViews, spotLights, cameras)));
                    }
                    ...
                    afx_msg void IDCameraView () {
                        if (editor.cameras.size () == 1) {
                            camera = editor.cameras[0];
                            currView = 'C';
                        }
                        else {
                            SelectCameraDialog selectCameraDialog (editor.cameras, this);
                            if (selectCameraDialog.DoModal () == IDOK) {
                                camera = editor.GetCamera ((LPCTSTR)selectCameraDialog.name);
                                currView = 'C';
                            }
                        }
                    }
                    ...
                    afx_msg void IDSpotLightView () {
                        if (GetSpotLightCount () == 1) {
                            ...
                            SelectSpotLightDialog selectSpotLightDialog (editor.lights, this);
                            if (selectSpotLightDialog.DoModal () == IDOK) {
                                spotLight = editor.GetLight ((LPCTSTR)selectSpotLightDialog.name);
                                currView = '$';
                            }
                        }
                    }
            ...
            IMPLEMENT_MENUBAR_ITEM_EVENT_HANDLER (MenuBarViewViewportsEventHandler, Editor)
