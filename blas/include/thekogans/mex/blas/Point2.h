    ...
    namespace mex {
        namespace blas {
            ...
            struct _LIB_THEKOGANS_MEX_BLAS_DECL Point2 {
                ...
                Point2 () {}
                Point2 (util::f32 x_, util::f32 y_) : x (x_), y (y_) {}
                Point2 (util::f32 xy[]) : x (xy[0]), y (xy[1]) {}
                ...
                inline Point2 Perpendicular () const {return Point2 (-y, x);}

                util::f32 AngleWithXAxis (util::f32 eps = util::EPSILON) const;
            ...
            inline Point2 operator * (util::f32 s, const Point2 &pt) {
                return Point2 (s * pt.x, s * pt.y);
            }

            inline Point2 operator * (const Point2 &pt, util::f32 s) {
            ...
            inline Point2 Mid (const Point2 &p1, const Point2 &p2) {
                return util::LERP (0.5f, p1, p2);
            }

            inline util::f32 Distance (const Point2 &p1, const Point2 &p2) {
                return (p1 - p2).Length ();
            }

            inline Point2 Normalize (const Point2 &pt, util::f32 eps = util::EPSILON) {
            ...
            inline util::f32 Dot (const Point2 &p1, const Point2 &p2) {
                return p1.x * p2.x + p1.y * p2.y;
            }
            ...
            inline util::Serializer &operator << (util::Serializer &serializer, const Point2 &pt) {
                serializer << pt.x << pt.y;
                return serializer;
            }

            inline util::Serializer &operator >> (util::Serializer &serializer, Point2 &pt) {
                serializer >> pt.x >> pt.y;
                return serializer;
            }
